...
EXAMPLES = r"""
 - name: lookup cloudformation output in the current region
   debug: msg="{{ lookup('amazon.aws.aws_cloudformation', 'stack_name', output_key='output_key' ) }}"
...
def stack_output(stack, output_key):
    ...
    if 'Outputs' in stack and len(stack['Outputs']) > 0:
        ...
        stack_output = next(filter(lambda output: output['OutputKey'] == output_key, outputs), None)
        if stack_output is not None and 'OutputValue' in stack_output:
            return stack_output['OutputValue']
        else:
            raise AnsibleError("Provided Cloudformation stack doesn't contain output with key: {}.".format(output_key))
    else:
    ...
    def run(self, terms, boto_profile=None, aws_profile=None,
        ...
        try:
            ...
            if 'Stack with id {} does not exist'.format(stack_name) in str(e):
                raise AnsibleError("Given stack doesn't exist: {}".format(stack_name))
            else:
